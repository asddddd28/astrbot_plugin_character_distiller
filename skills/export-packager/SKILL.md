...
Memorix 直接写入要求 AstrBot 中已加载 `astrbot_plugin_memorix`。如果直接写入失败，插件会生成 `rag_export/memorix_import/<角色>/memorix_import_<角色>_<work_id>.json`，可在 Memorix 导入中心启用后手动导入。
...
10. `rag_export/memorix_import/<角色>/memorix_import_<角色>_<work_id>.json`
